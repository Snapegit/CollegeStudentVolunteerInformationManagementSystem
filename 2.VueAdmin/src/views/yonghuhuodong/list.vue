...
<template>
	<div>
			...
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.huodongdidian" placeholder="活动地点"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('yonghuhuodong','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('yonghuhuodong','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('yonghuhuodong','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('yonghuhuodong','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('yonghuhuodong','活动报名统计')">活动报名统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('yonghuhuodong','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('yonghuhuodong','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('yonghuhuodong','记录')" type="success" @click="huodongjiluCrossAddOrUpdateHandler(scope.row,'cross','是','','')">记录</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'活动报名统计'" width="70%">
			<div  id="baomingrenshuEchart1" style="width:100%;height:600px;"></div>
		...
		<huodongjiluFormModel ref="huodongjiluFormModelRef" @formModelChange="formModelChange"></huodongjiluFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.huodongdidian&&searchQuery.value.huodongdidian!=''){
			params['huodongdidian'] = '%' + searchQuery.value.huodongdidian + '%'
		}
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var baomingrenshuEchart1 = echarts.init(document.getElementById("baomingrenshuEchart1"),'macarons');
			context?.$http({
				url: `yonghuhuodong/value/huodongmingcheng/baomingrenshu`,
				method: 'get'
			}).then(res=>{
				...
				// 使用刚指定的配置项和数据显示图表。
				baomingrenshuEchart1.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    baomingrenshuEchart1.resize();
				};
			})
		})
	}
	...
	import huodongjiluFormModel from '@/views/huodongjilu/formModel'
	const huodongjiluFormModelRef = ref(null)
    const huodongjiluCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			huodongjiluFormModelRef.value.init(row.id,'cross','记录',row,'yonghuhuodong',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
